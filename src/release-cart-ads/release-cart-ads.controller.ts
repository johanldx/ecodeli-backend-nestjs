...
export class ReleaseCartAdsController {
  ...
  @Post()
  @ApiResponse({ status: 201, description: 'Release Cart Ad created successfully', type: ReleaseCartAdResponseDto })
  async create(@Body() dto: CreateReleaseCartAdDto, @CurrentUser() user: User) {
    return this.releaseCartAdsService.create(dto, user);  
  }

  
  @Get()
  @ApiResponse({ status: 200, description: 'Returns the list of release cart ads', type: [ReleaseCartAdResponseDto] })
  async findAll(@Query() query: any, @CurrentUser() user: User) {
    return this.releaseCartAdsService.findAll(user, query);  
  }

  @Get(':id')
  @ApiResponse({ status: 200, description: 'Returns the release cart ad by ID', type: ReleaseCartAdResponseDto })
  ...
  @Patch(':id')
  @ApiResponse({ status: 200, description: 'Release Cart Ad updated successfully', type: ReleaseCartAdResponseDto })
  async update(@Param('id') id: number, @Body() dto: UpdateReleaseCartAdDto, @CurrentUser() user: User) {
    return this.releaseCartAdsService.update(id, dto, user); 
  }

  @Delete(':id')
  @ApiResponse({ status: 200, description: 'Release Cart Ad deleted successfully' })
